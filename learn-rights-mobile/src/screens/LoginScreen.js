...
const LoginScreen = ({ navigation }) => {
  ...
  useEffect(() => {
    if (response) {
      console.log("Google Login Full Response:", JSON.stringify(response, null, 2));
      
      if (response.type === 'success') {
        const { authentication } = response;
        console.log("Google Login SUCCESS. Token obtained:", authentication.accessToken ? "YES" : "NO");
        handleGoogleLogin(authentication.accessToken);
      } else {
        console.log("Google Login RESPONSE NOT SUCCESS:", response.type);
        if (response.type === 'error' || response.type === 'cancel' || response.type === 'dismiss') {
          console.warn(`Google Login ${response.type}:`, response);
